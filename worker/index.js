  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (url.pathname === "/api/admin/me" && request.method === "PUT") {
        ...
        if (!username || !displayName) throw new HttpError(400, "à¸à¸£à¸¸à¸à¸²à¸à¸£à¸­à¸à¸à¸·à¹à¸­à¸à¸¹à¹à¹à¸à¹à¹à¸¥à¸°à¸à¸·à¹à¸­à¸à¸µà¹à¹à¸ªà¸à¸");
        ...
        catch (error) { if (/unique/i.test(String(error))) throw new HttpError(409,"à¸à¸·à¹à¸­à¸à¸¹à¹à¹à¸à¹à¸à¸µà¹à¸¡à¸µà¸­à¸¢à¸¹à¹à¹à¸¥à¹à¸§");throw error; }
        ...
        return json({ success:true, user:publicUser(user), message:"à¸à¸±à¸à¸à¸¶à¸à¹à¸à¸£à¹à¸à¸¥à¹à¹à¸£à¸µà¸¢à¸à¸£à¹à¸­à¸¢" }, 200, cors);
      }
      ...
      if (url.pathname === "/api/admin/users" && request.method === "POST") {
        ...
        try {
          ...
          return json({ success:true, id:result.meta?.last_row_id, message:"à¸ªà¸£à¹à¸²à¸à¸à¸±à¸à¸à¸µà¹à¸£à¸µà¸¢à¸à¸£à¹à¸­à¸¢" }, 201, cors);
        } catch (error) {
          if (/unique/i.test(String(error))) throw new HttpError(409, "à¸à¸·à¹à¸­à¸à¸¹à¹à¹à¸à¹à¸à¸µà¹à¸¡à¸µà¸­à¸¢à¸¹à¹à¹à¸¥à¹à¸§");
          throw error;
        }
      }
      ...
      if (userMatch && request.method === "PUT") {
        ...
        if (!target) throw new HttpError(404, "à¹à¸¡à¹à¸à¸à¸à¸±à¸à¸à¸µà¸à¸¹à¹à¹à¸à¹");
        const data = await readJson(request);
        if (target.role === "owner" && ((data.role!==undefined && normalizeRole(data.role)!=="owner") || data.active===false)) {
          ...
          if (Number(owners?.total||0)<=1) throw new HttpError(400,"à¸à¹à¸­à¸à¸¡à¸µà¸à¸¹à¹à¸à¸¹à¹à¸¥à¸«à¸¥à¸±à¸à¸à¸µà¹à¹à¸à¸´à¸à¹à¸à¹à¸à¸²à¸à¸­à¸¢à¹à¸²à¸à¸à¹à¸­à¸¢ 1 à¸à¸±à¸à¸à¸µ");
        }
        const fields = [], values = [];
        if (data.display_name !== undefined) { const v=cleanText(data.display_name);if(!v)throw new HttpError(400,"à¸à¸·à¹à¸­à¸à¸µà¹à¹à¸ªà¸à¸à¸«à¹à¸²à¸¡à¸§à¹à¸²à¸");fields.push("display_name=?");values.push(v); }
        if (data.role !== undefined) { fields.push("role=?");values.push(normalizeRole(data.role)); }
        if (data.active !== undefined) {
          if (id===session.id && !data.active) throw new HttpError(400,"à¸à¸´à¸à¸à¸±à¸à¸à¸µà¸à¸µà¹à¸à¸³à¸¥à¸±à¸à¹à¸à¹à¸à¸²à¸à¹à¸¡à¹à¹à¸à¹");
        ...
        if (!fields.length) throw new HttpError(400, "à¹à¸¡à¹à¸¡à¸µà¸à¹à¸­à¸¡à¸¹à¸¥à¸à¸µà¹à¸à¹à¸­à¸à¹à¸à¹à¹à¸");
        ...
        return json({ success:true, message:"à¹à¸à¹à¹à¸à¸à¸±à¸à¸à¸µà¹à¸£à¸µà¸¢à¸à¸£à¹à¸­à¸¢" }, 200, cors);
      }
      ...
      if (url.pathname === "/api/admin/social-links" && request.method === "POST") {
        ...
        return json({success:true,id:result.meta?.last_row_id,message:"à¸à¸±à¸à¸à¸¶à¸à¸¥à¸´à¸à¸à¹à¹à¸£à¸µà¸¢à¸à¸£à¹à¸­à¸¢"},201,cors);
      }
      ...
      if(socialMatch){
        ...
        if(request.method==="PUT"){const data=validateSocialLink(await readJson(request));const result=await env.DB.prepare("UPDATE site_social_links SET platform=?,label=?,url=?,active=?,show_footer=?,show_article=?,sort_order=?,updated_at=datetime('now') WHERE id=?").bind(data.platform,data.label,data.url,data.active?1:0,data.show_footer?1:0,data.show_article?1:0,data.sort_order,id).run();if(!result.meta?.changes)throw new HttpError(404,"à¹à¸¡à¹à¸à¸à¸¥à¸´à¸à¸à¹");await writeAudit(env.DB,session,"update_social_link","social_link",id,data.platform);return json({success:true,message:"à¹à¸à¹à¹à¸à¸¥à¸´à¸à¸à¹à¹à¸£à¸µà¸¢à¸à¸£à¹à¸­à¸¢"},200,cors)}
        if(request.method==="DELETE"){const result=await env.DB.prepare("DELETE FROM site_social_links WHERE id=?").bind(id).run();if(!result.meta?.changes)throw new HttpError(404,"à¹à¸¡à¹à¸à¸à¸¥à¸´à¸à¸à¹");await writeAudit(env.DB,session,"delete_social_link","social_link",id,"");return json({success:true,message:"à¸¥à¸à¸¥à¸´à¸à¸à¹à¹à¸£à¸µà¸¢à¸à¸£à¹à¸­à¸¢"},200,cors)}
      }
      ...
      if(authorMatch&&request.method==="GET"){
        ...
        const user=await env.DB.prepare("SELECT * FROM admin_users WHERE username=? COLLATE NOCASE AND active=1 LIMIT 1").bind(username).first();if(!user)throw new HttpError(404,"à¹à¸¡à¹à¸à¸à¸à¸¹à¹à¹à¸à¸µà¸¢à¸");
      ...
      if (url.pathname === "/api/admin/articles" && request.method === "POST") {
        ...
        return json({ success: true, id: result.meta?.last_row_id, message: "à¸à¸±à¸à¸à¸¶à¸à¸à¸à¸à¸§à¸²à¸¡à¹à¸£à¸µà¸¢à¸à¸£à¹à¸­à¸¢" }, 201, cors);
      }
      ...
      if (adminMatch) {
        ...
        if (request.method === "PUT") {
          ...
          if (!existing) throw new HttpError(404, "à¹à¸¡à¹à¸à¸à¸à¸à¸à¸§à¸²à¸¡");
          if (session.role !== "owner" && Number(existing.author_user_id)!==Number(session.id)) throw new HttpError(403, "à¹à¸à¹à¹à¸à¹à¸à¹à¹à¸à¸à¸²à¸°à¸à¸à¸à¸§à¸²à¸¡à¸à¸­à¸à¸à¸±à¸§à¹à¸­à¸");
          const data = await readJson(request);
          validateArticle(data);
          const status = normalizeStatus(data.status);
          const publishedAt = getPublicationTime(status, data.publish_at);
          ...
          return json({ success: true, message: "à¹à¸à¹à¹à¸à¸à¸à¸à¸§à¸²à¸¡à¹à¸£à¸µà¸¢à¸à¸£à¹à¸­à¸¢" }, 200, cors);
        }
        if (request.method === "DELETE") {
          const existing = await env.DB.prepare("SELECT author_user_id FROM articles WHERE id=?").bind(id).first();
          if (existing && session.role !== "owner" && Number(existing.author_user_id)!==Number(session.id)) throw new HttpError(403, "à¸¥à¸à¹à¸à¹à¹à¸à¸à¸²à¸°à¸à¸à¸à¸§à¸²à¸¡à¸à¸­à¸à¸à¸±à¸§à¹à¸­à¸");
          const result = await env.DB.prepare("DELETE FROM articles WHERE id = ?").bind(id).run();
          if (!result.meta?.changes) throw new HttpError(404, "à¹à¸¡à¹à¸à¸à¸à¸à¸à¸§à¸²à¸¡");
          await writeAudit(env.DB,session,"delete_article","article",id,"");
          return json({ success: true, message: "à¸¥à¸à¸à¸à¸à¸§à¸²à¸¡à¹à¸£à¸µà¸¢à¸à¸£à¹à¸­à¸¢" }, 200, cors);
        }
      }
      ...
      if (url.pathname === "/api/admin/popup" && request.method === "PUT") { await requireAdmin(request,env);await ensurePopupTable(env.DB);const d=await readJson(request);if(d.enabled&&!cleanText(d.image_url))throw new HttpError(400,"à¸à¸£à¸¸à¸à¸²à¸£à¸°à¸à¸¸ URL à¸£à¸¹à¸ Popup");const s=nullableIso(d.start_at),e=nullableIso(d.end_at);if(s&&e&&new Date(e)<=new Date(s))throw new HttpError(400,"à¹à¸§à¸¥à¸²à¸ªà¸´à¹à¸à¸ªà¸¸à¸à¸à¹à¸­à¸à¸­à¸¢à¸¹à¹à¸«à¸¥à¸±à¸à¹à¸§à¸¥à¸²à¹à¸£à¸´à¹à¸¡");await env.DB.prepare(`INSERT INTO site_popup(id,enabled,title,image_url,link_url,start_at,end_at,once_per_session,updated_at) VALUES(1,?,?,?,?,?,?,?,datetime('now')) ON CONFLICT(id) DO UPDATE SET enabled=excluded.enabled,title=excluded.title,image_url=excluded.image_url,link_url=excluded.link_url,start_at=excluded.start_at,end_at=excluded.end_at,once_per_session=excluded.once_per_session,updated_at=datetime('now')`).bind(d.enabled?1:0,nullableText(d.title),nullableText(d.image_url),nullableText(d.link_url),s,e,d.once_per_session===false?0:1).run();return json({success:true,message:"à¸à¸±à¸à¸à¸¶à¸ Popup à¹à¸£à¸µà¸¢à¸à¸£à¹à¸­à¸¢"},200,cors); }
      ...
      if (publicMatch && request.method === "GET") {
        ...
        `).bind(slug, now).first();
        if (!article) throw new HttpError(404, "à¹à¸¡à¹à¸à¸à¸à¸à¸à¸§à¸²à¸¡");
      ...
      console.error(error);
      return json({ success: false, message: "à¹à¸à¸´à¸à¸à¹à¸­à¸à¸´à¸à¸à¸¥à¸²à¸à¹à¸à¸£à¸°à¸à¸" }, 500, cors);
    }
  }
};
...
async function handleLogin(request, env, cors) {
  ...
  if (!user || !user.active || !(await verifyPassword(body.password || "", user.password_salt, user.password_hash))) {
    throw new HttpError(401, "à¸à¸·à¹à¸­à¸à¸¹à¹à¹à¸à¹à¸«à¸£à¸·à¸­à¸£à¸«à¸±à¸ªà¸à¹à¸²à¸à¹à¸¡à¹à¸à¸¹à¸à¸à¹à¸­à¸");
  }
...
async function requireAdmin(request, env) {
  ...
  if (!payload?.sub) {
    throw new HttpError(401, "Session à¸«à¸¡à¸à¸­à¸²à¸¢à¸¸ à¸à¸£à¸¸à¸à¸²à¹à¸à¹à¸²à¸ªà¸¹à¹à¸£à¸°à¸à¸à¹à¸«à¸¡à¹");
  }
  const user = await env.DB.prepare("SELECT * FROM admin_users WHERE id=? LIMIT 1").bind(payload.sub).first();
  if (!user || !user.active) throw new HttpError(401, "à¸à¸±à¸à¸à¸µà¸à¸¹à¸à¸à¸´à¸à¹à¸à¹à¸à¸²à¸à¸«à¸£à¸·à¸­ Session à¸«à¸¡à¸à¸­à¸²à¸¢à¸¸");
  return user;
}

async function requireOwner(request, env) {
  const user = await requireAdmin(request, env);
  if (user.role !== "owner") throw new HttpError(403, "à¸ªà¸³à¸«à¸£à¸±à¸à¸à¸¹à¹à¸à¸¹à¹à¸¥à¸«à¸¥à¸±à¸à¹à¸à¹à¸²à¸à¸±à¹à¸");
  return user;
}
...
async function ensureAdminUsers(db) {
  ...
  const additions=[["bio","TEXT"],["avatar_url","TEXT"],["facebook_url","TEXT"],["tiktok_url","TEXT"],["youtube_url","TEXT"],["x_url","TEXT"]];
...
function getPublicationTime(status, publishAt) {
  ...
  if (Number.isNaN(date.getTime())) throw new HttpError(400, "à¸à¸£à¸¸à¸à¸²à¸£à¸°à¸à¸¸à¹à¸§à¸¥à¸²à¹à¸à¸¢à¹à¸à¸£à¹à¸à¸µà¹à¸à¸¹à¸à¸à¹à¸­à¸");
...
function normalizeArticle(a) {
  ...
  const fallbackDesc = cleanText(a.excerpt) || (type === "news" ? "à¸à¹à¸²à¸§à¸à¸¸à¸à¸à¸­à¸¥à¸à¸²à¸ HN FOOTBALL SCORE" : type === "evergreen" ? "à¸à¸§à¸²à¸¡à¸£à¸¹à¹à¸à¸¸à¸à¸à¸­à¸¥à¸à¸²à¸ HN FOOTBALL SCORE" : "à¸à¸à¸§à¸´à¹à¸à¸£à¸²à¸°à¸«à¹à¸à¸¸à¸à¸à¸­à¸¥à¸à¸²à¸ HN FOOTBALL SCORE");
  ...
  return {
    ...
    author: a.author || "à¸à¸­à¸ª à¸ªà¸´à¸à¸à¸´à¸à¸£",
...
function validateArticle(data) {
  if (!cleanText(data.title)) throw new HttpError(400, "à¸à¸£à¸¸à¸à¸²à¸à¸£à¸­à¸à¸«à¸±à¸§à¸à¹à¸­à¸à¸à¸à¸§à¸²à¸¡");
  if (!cleanText(data.content)) throw new HttpError(400, "à¸à¸£à¸¸à¸à¸²à¸à¸£à¸­à¸à¹à¸à¸·à¹à¸­à¸«à¸²à¸à¸à¸à¸§à¸²à¸¡");
  if (String(data.title).length > 160) throw new HttpError(400, "à¸«à¸±à¸§à¸à¹à¸­à¸à¸à¸à¸§à¸²à¸¡à¸¢à¸²à¸§à¹à¸à¸´à¸à¹à¸");
}
...
function validatePassword(password) { if (String(password||"").length<8) throw new HttpError(400,"à¸£à¸«à¸±à¸ªà¸à¹à¸²à¸à¸à¹à¸­à¸à¸¡à¸µà¸­à¸¢à¹à¸²à¸à¸à¹à¸­à¸¢ 8 à¸à¸±à¸§"); }
...
function normalizeWikimedia(username, value) {
  ...
  let url;try{url=new URL(raw)}catch{throw new HttpError(400,"à¸¥à¸´à¸à¸à¹ Wikimedia à¹à¸¡à¹à¸à¸¹à¸à¸à¹à¸­à¸")}
  const host=url.hostname.toLowerCase();
  if (!(host.endsWith("wikimedia.org")||host.endsWith("wikipedia.org")||host.endsWith("wikidata.org"))) throw new HttpError(400,"à¸à¸£à¸¸à¸à¸²à¹à¸à¹à¸¥à¸´à¸à¸à¹ Wikimedia, Wikipedia à¸«à¸£à¸·à¸­ Wikidata à¹à¸à¹à¸²à¸à¸±à¹à¸");
...
async function handleUpload(request, env, cors, session) {
  if (!env.MEDIA) throw new HttpError(500,"à¸¢à¸±à¸à¹à¸¡à¹à¹à¸à¹à¸à¸¹à¸ R2 bucket à¸à¸·à¹à¸­ MEDIA");
  const form=await request.formData();const file=form.get("file");
  if (!(file instanceof File)) throw new HttpError(400,"à¸à¸£à¸¸à¸à¸²à¹à¸¥à¸·à¸­à¸à¸£à¸¹à¸");
  const allowed={"image/jpeg":"jpg","image/png":"png","image/webp":"webp","image/gif":"gif"};
  const ext=allowed[file.type];if(!ext)throw new HttpError(400,"à¸£à¸­à¸à¸£à¸±à¸à¹à¸à¸à¸²à¸° JPG, PNG, WebP à¹à¸¥à¸° GIF");
  if(file.size>5*1024*1024)throw new HttpError(413,"à¸£à¸¹à¸à¸à¹à¸­à¸à¸¡à¸µà¸à¸à¸²à¸à¹à¸¡à¹à¹à¸à¸´à¸ 5 MB");
...
async function serveMedia(request, env, cors) {
  ...
  const object=await env.MEDIA.get(key);if(!object)throw new HttpError(404,"à¹à¸¡à¹à¸à¸à¸£à¸¹à¸");
...
async function readJson(request) {
  ...
  catch { throw new HttpError(400, "à¸£à¸¹à¸à¹à¸à¸à¸à¹à¸­à¸¡à¸¹à¸¥à¹à¸¡à¹à¸à¸¹à¸à¸à¹à¸­à¸"); }
}
...
function nullableIso(v){if(!v)return null;const d=new Date(v);if(Number.isNaN(d.getTime()))throw new HttpError(400,"à¸£à¸¹à¸à¹à¸à¸à¸§à¸±à¸à¸à¸µà¹à¸«à¸£à¸·à¸­à¹à¸§à¸¥à¸²à¹à¸¡à¹à¸à¸¹à¸à¸à¹à¸­à¸");return d.toISOString();}
